...
public class JavaParser implements JavaParserConstants {
   ...
   private boolean isaMethod = false;
  ...
  public void analyzeFile(String path){
	  JavaParser parser;
	  System.out.println("Java Parser Version 1.7:  Reading from file " + path + "...");
      try {
  ...
  private static JavaParser ps=null;
	private JavaParser(){}
	public static JavaParser getInstance(){
		if(ps==null)ps=new JavaParser();
		return ps;
	}
  
  public void cleanArrayFile(){
	  array.clear();
  }
...
// Handle classes, interfaces, enums, and annotations.
  ...
  final public void ClassOrInterfaceDeclaration() throws ParseException {
    ...
    arrayAux.add("code:ClassUnit");
    arrayAux.add(token.image);
    arrayAux.add(Integer.toString(token.beginLine));
    for (int i = 0; i < arrayAux.size(); i++) {
		array.add(arrayAux.get(i));
	}
    arrayAux.clear();
    System.out.println("Clase ==> "+token.image+" en linea: "+token.beginLine);
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case LT:{
      TypeParameters();
      break;
    ...
    writeFile(ResourcesPlugin.getWorkspace().getRoot().getLocation()+"\\Resources\\output.txt", array);
    if (jj_2_4(2)) {
      jj_consume_token(SEMICOLON);    
    } else {
      ;
  ...
  final public void VariableDeclarator() throws ParseException {
    ...
    if(!this.isaMethod){
    	arrayAux.add("code:StorableUnit");
        arrayAux.add(token.image);
        arrayAux.add(Integer.toString(token.beginLine));
        for (int i = 0; i < arrayAux.size(); i++) {
    		array.add(arrayAux.get(i));
        ...
        arrayAux.clear();
    	System.out.println("Atributo ==> "+token.image+" en linea: "+token.beginLine);
    }
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case ASSIGN:{
  ...
  final public void MethodDeclaration() throws ParseException {
    ...
    this.isaMethod = false;
  }

  final public void MethodDeclarator() throws ParseException {
    jj_consume_token(IDENTIFIER);
    if(isaMethod){
    	arrayAux.add("code:MethodUnit");
        arrayAux.add(token.image);
        arrayAux.add(Integer.toString(token.beginLine));
        for (int i = 0; i < arrayAux.size(); i++) {
    		array.add(arrayAux.get(i));
        ...
        arrayAux.clear();
    	System.out.println("Metodo==>"+token.image+" en linea: "+token.beginLine+"\n");
    }
    FormalParameters();
    label_14:
    while (true) {
...
void RUNSIGNEDSHIFT() throws ParseException {
    if (getToken(1).kind == GT &&
                     getToken(1) instanceof Token &&
                    ((Token)getToken(1)).kind == RUNSIGNEDSHIFT) {
  ...
  final public void RSIGNEDSHIFT() throws ParseException {
    if (getToken(1).kind == GT &&
                     getToken(1) instanceof Token &&
                    ((Token)getToken(1)).kind == RSIGNEDSHIFT) {
